...
Go to ressource folder, and print needed markers. In this folder, you'll also
find yml files needed to run the executables with these markers.

If you interested in designing new markers, please refer to the complete guide
...
\end{verbatim}
Then, because it implements IA, you need to override getCommand (in cpp this
kind of function is called pure virtual, virtual meaning undefined and pure
meaning it has to be redifined in every sub class).
\begin{verbatim}
	virtual void getCommand(vector<aruco::Marker>* TheMarkers, int* steering,
    int* throttle, int width);
\end{verbatim}
getCommand will change steering and thrttle values. You can use the marker
...
\begin{itemize}
    \item Add your IA in the IA enum called Runmode at the beginning of the
        program.
    \item Add your IA in readParams to change the runmode according to you IA.
    \item Add your IA in choose\_run\_mode to instanciate it properly.
\end{itemize}
